...
public:
	...
	Date(int new_year, int new_month, int new_day) {
		if (new_month > 12 || new_month < 1 || new_day > 31 || new_day < 1) {
...
void сheckSymbol(istream& stream) {
	if (stream.peek() != '-')
...
public:
	...
	void deleteEvent(const Date& date, const string& event) {
		if (calendar.count(date)) {
			if (calendar[date].count(event)) {
				calendar[date].erase(event);
				cout << "Event has been erased successfully" << endl;
			}
		} else {
	...
	void deleteDate(const Date& date) {
		...
		if (calendar.count(date)) 
		{
			event_count = calendar[date].size();
			calendar.erase(date);
		}
		cout << event_count << " events were deleted" << endl;
	}

	void findDate(const Date& date) {
		if (calendar.count(date)) {
			for (auto& i : calendar[date]) {
				cout << i << endl;
			}
		}
	}
...
int mainFP1() {
	...
	while (command != "exit") {
		...
		cin >> command;
		if (command == "add") {
			cout << "Format: YYYY-MM-DD-EVENT" << endl;
			Date date;
			string event;
			try {
				cin >> date >> event;
